...
import { IONLYV3Pool, IWETH9, MockTimeSwapRouter, TestERC20 } from '../typechain'
...
import { abi as IONLYV3PoolABI } from '@ekawibisono/v3-core/artifacts/contracts/interfaces/IONLYV3Pool.sol/IONLYV3Pool.json'
...
describe('SwapRouter gas tests', function () {
  ...
  const swapRouterFixture: Fixture<{
    ...
    pools: [IONLYV3Pool, IONLYV3Pool, IONLYV3Pool]
    ...
    const pools = poolAddresses.map((poolAddress) => new ethers.Contract(poolAddress, IONLYV3PoolABI, wallet)) as [
      IONLYV3Pool,
      IONLYV3Pool,
      IONLYV3Pool
    ]
  ...
  let pools: [IONLYV3Pool, IONLYV3Pool, IONLYV3Pool]
  ...
  describe('#exactInput', () => {
    ...
    it('0 -> 1 minimal', async () => {
      const calleeFactory = await ethers.getContractFactory('TestONLYV3Callee')
